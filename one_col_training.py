...
def save_acc(accuracy, name_acc):
    with open(str(parse_args().output_path) + "/kfold_acc_model_1D.txt", 'a') as f:
...
def main():
    ...
    if dataset['type'] != "Model_1D":
        print("Preprocessing data: Model_1D")
        os.system("python preprocess_data.py --colapsed")
        dataset = load_from_bin(args.data_path.replace('.vcf.gz', '.bin'))
    ...
    for train_idx, val_idx in list(kfold.split(sample_feature_2D, label_1D)):
        ...
        trainer = Trainer(
            model=SharedNet1D(dataset['input-size'], dataset['outputs-size']),
